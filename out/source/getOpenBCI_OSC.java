...
public class getOpenBCI_OSC extends PApplet {
...
public void drawOnRadius(int angle,int channel,int band) {
 int limit = 200;
 int factor = 150;
    ...
    if (receivedInPacket == packetSize) {
        for (int channel = 1;channel <= nbChannelMax; channel++) {
            ...
            for (int band = 0;band < 5;band++) {
                ...
                aPowerVectorCumul[channel - 1][band] = 0.0f;
          }
...
public void traceMessage(String context, int channel, float[][] aVector) {
    if (channel!= 1) return;
...
public @Override void exit() {
  ...
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "--present", "--window-color=#666666", "--stop-color=#cccccc", "getOpenBCI_OSC" };
    if (passedArgs != null) {
